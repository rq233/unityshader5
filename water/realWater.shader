...
Shader "Unlit/realWater4"
{
    Properties{
        ...
        //水的反射
        _RelfectAmount("Relfect Amount",Range(0,4)) = 1
    ...
    SubShader{
        ...
        pass{
            ...
            fixed4 frag (v2f i ):SV_TARGET{
                ...
                /* color.xy *= _BumpScale;
                color.z = sqrt(1.0-saturate(dot(color.xy, color.xy)));
                color = float3(color.xy, color.z); */
                //给于两层是为了体现上下波动的效果
                
                fixed uM = color.r;
                fixed vM = color.r;
                float4 Noicol = tex2D(_WaveMap, i.uv + float2(uM, vM));
                //这里我们用给经处理过的噪点，将其处理，让它成为maintex采样的uv
                
                float3 albedo = tex2D(_MainTex, i.uv).rgb;
                float3 ambient = UNITY_LIGHTMODEL_AMBIENT.xyz * albedo;
                float3 diffuse = _Color.xyz * albedo  * max(0, dot(worldNormal, worldLightDir));
                float3 specular = pow(max(0, dot(worldNormal* color.xyz, worldView)),_Gloss) * ambient * _Specular.rgb;
                fixed3 reflection = texCUBE(_Cube,i.worldRefl + color.xyz).rgb * _ReflectColor.rgb;
                fixed fresnel = _FresnelScale + (1 - _FresnelScale )*pow(1-dot(worldView, worldNormal*color.xyz),5);
                float3 col =  lerp(diffuse,reflection,saturate(fresnel)).xyz;
                float2 offset = color * (_DistortionFactor * 10) * _GrabTexture_TexelSize.xy * 10;
                ...
                return fixed4 (col + specular +diffuse + dis.rgb, 0.5);
            }ENDCG
        }
    }Fallback "Diffuse"
}